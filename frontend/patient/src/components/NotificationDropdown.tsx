...
import { BellIcon, ChatBubbleLeftRightIcon, CalendarDaysIcon, XMarkIcon, CheckIcon } from '@heroicons/react/24/outline';
...
const NotificationDropdown: React.FC = () => {
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="relative p-2 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-full">
          ...
          <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-20">
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                ...
                <button onClick={clearAllNotifications} className="text-sm text-gray-500 hover:text-gray-700">Clear all</button>
              </div>
            </div>
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                <div className="divide-y divide-gray-200">
                  {notifications.map((n) => (
                    <div key={n.id} className={`p-4 hover:bg-gray-50 ${!n.read ? 'bg-blue-50' : ''}`}>
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {!n.read && (
                            <button onClick={() => markAsRead(n.id)} className="mt-2 text-xs text-blue-600 hover:text-blue-800 flex items-center">
                              <CheckIcon className="h-3 w-3 mr-1" /> Mark as read
                            </button>
                          )}
                        </div>
                      </div>
